...
describe('SearchResults mileage filter behavior', () => {
  afterEach(() => {
    cleanup();
  });
  ...
  it('applies min and max mileage values to listing filters', async () => {
    const user = userEvent.setup();
    render(<SearchResults />);

    const minMileageInput = await screen.findByPlaceholderText('Min km');
    const maxMileageInput = await screen.findByPlaceholderText('Max km');

    await user.type(minMileageInput, '50000');
    await user.type(maxMileageInput, '100000');
